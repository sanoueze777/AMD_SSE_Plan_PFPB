...
st.set_page_config(
    page_title="BIG DATA MULTIPAYS | BASE DE DONNEES",
    layout='wide',
...
sidebar_selection = st.sidebar.radio(
    ...
    ['Selectionner les pays', 'California'],
)
...
def plot_county(county):
    ...
    if len(county) == 1:
        st.subheader('DONNEES SOCIOECONOMIQUES '+', '.join(map(str, county))+' county ('+ str(today)+')')
    else:
        st.subheader('DONNEES SOCIOECONOMIQUES '+', '.join(map(str, county))+' counties ('+ str(today)+')')
...
t1, t2 = st.columns(2)
with t1:
    st.markdown('# BIG DATA MULTIPAYS')
...
if sidebar_selection == 'Selectionner les pays':
    st.markdown("## Selectionner les pays d'intéret")
